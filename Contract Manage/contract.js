...
function send() {
    ...
    document.getElementById("contractForm").addEventListener("submit", async function(event) {
      ...
      const response = await fetch("https://api-production-e66f.up.railway.app/api/project/createProject", {
          method: "POST",
          body: formData
      });
...
document.addEventListener("DOMContentLoaded",  function() {
  // Fetch worker data from API
   fetch('https://api-production-e66f.up.railway.app/api/project/')
...
document.addEventListener("DOMContentLoaded",  function() {
  // Fetch worker data from API
    fetch('https://api-production-e66f.up.railway.app/api/attendance/')
    .then(response => response.json())
    .then(data => {
